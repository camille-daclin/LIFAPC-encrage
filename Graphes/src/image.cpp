...
Image::Image(const string& fichier, float sig, float alp, int h)
    : sigma(sig), alpha(alp), H(h)
{
    ...
    // ATTENTION: dans PGM c'est LARGEUR puis HAUTEUR !
    ...
    // Lecture des intensités (ligne par ligne, de gauche à droite)
...
int Image::chercher_chemin_ameliorant(int* predecesseur, Direction* dir_pred)
{
    ...
    while (!file.empty() && !puit_atteint) {
        ...
        // Si on est à la source, explorer tous les pixels
        ...
        // Si on est sur un pixel
            ...
            int voisins[4] = {voisin_nord(courant), voisin_sud(courant), 
                              voisin_est(courant), voisin_ouest(courant)};
            Direction dirs[4] = {NORD, SUD, EST, OUEST};
            unsigned int caps[4] = {pixels[courant].get_capacite_nord(),
                                   pixels[courant].get_capacite_sud(),
                                   pixels[courant].get_capacite_est(),
                                   pixels[courant].get_capacite_ouest()};
            unsigned int flots[4] = {pixels[courant].get_flot_nord(),
                                    pixels[courant].get_flot_sud(),
                                    pixels[courant].get_flot_est(),
                                    pixels[courant].get_flot_ouest()};
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if (voisin != -1 && !visite[voisin]) {
                    ...
                    switch(dir_opp) {
                        case NORD: flot_retour = pixels[voisin].get_flot_nord(); break;
                        case SUD: flot_retour = pixels[voisin].get_flot_sud(); break;
                        case EST: flot_retour = pixels[voisin].get_flot_est(); break;
                        case OUEST: flot_retour = pixels[voisin].get_flot_ouest(); break;
                        default: break;
                    }
...
void Image::augmenter_flot(int* predecesseur, Direction* dir_pred, int idx_puit)
{
    ...
    while (courant != idx_source) {
        ...
        if (pred == idx_source) {
            ...
            // Arc pixel -> pixel avec gestion de l'excédent
...
void Image::calculer_flot_maximal()
{
    ...
    cout << "Calcul du flot maximal..." << endl;
...
{
    // Parcours depuis la source pour marquer les pixels atteignables
    int taille_totale = hauteur * largeur + 2;
    ...
    while (!file.empty()) {
        ...
        if (courant == idx_source) {
            // Explorer tous les pixels depuis la source
            for (int idx = 0; idx < hauteur * largeur; idx++) {
                unsigned int cap = pixels[idx].get_capacite_source();
                unsigned int flot = pixels[idx].get_flot_source();
    ...
    for (int idx = 0; idx < hauteur * largeur; idx++) {
        if (connecte_source[idx]) {
            pixels[idx].set_niveau_gris(0);  // Noir
        } else {
            pixels[idx].set_niveau_gris(255);  // Blanc
        }
    }
...
void Image::sauvegarder(const string& fichier) const
{
    ...
    file << "P2" << endl;
    file << largeur << " " << hauteur << endl;  // LARGEUR puis HAUTEUR
    file << "255" << endl;
