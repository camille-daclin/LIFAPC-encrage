...
void Image::augmenter_flot(int* predecesseur, Direction* dir_pred, int idx_puit)
{
    ...
    while (courant != idx_source)
    {
        ...
        if (pred == idx_source)
        {
            cap_residuelle = pixels[courant].get_capacite_source() - pixels[courant].get_flot_source(); // Source -> Pixel
        }
        else if (courant == idx_puit)
        {
            cap_residuelle = pixels[pred].get_capacite_puit() -  pixels[pred].get_flot_puit(); // Pixel -> Puit
        }
        else
        {
            unsigned int cap = 0, flot = 0, flot_retour = 0; //Pixel -> Pixel           
            switch(dir)
            {
                case NORD:
